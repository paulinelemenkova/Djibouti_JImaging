...
Technologies in {image analysis} play the essential role in development {of the environmental monitoring}. {Traditional tools of processing RS data reply on} using {the} \hl{GIS} {which enable to extract geospatial information from the satellite images} \cite{BORANA2023261,Zhu02012021}. {Recently, a} number of {the advanced} techniques have been developed for land cover change detection. {To mention a few of them,} image enhancement \cite{Dyson2024}, image differentiation, rationing or principal component analysis methods \cite{7877513} and {more}. However, to obtain detailed information on land cover changes {using time series analysis, automation} of image {analysis} is needed {due to large volumes of RS data}. 

 {Recent progress in Machine Learning (ML) methods increased the efficiency of such techniques through automated algorithms of image analysis. The application of ML} utilise training datasets as a seed for classification \cite{FERCHICHI2024122211}. {Existing ML algorithms include, for instance, Random Forest (RF)} \cite{Balha2022,Lemenkova202401}, {Support Vector Machines (SVM)} \cite{10450641}, {or Convolutional Neural Networks (CNN)} \cite{SUAREZFERNANDEZ2024}. Such {approaches are derived} from programming{ and effectively adapted to satellite image analysis}. Automated methods of ML are increasingly needed for spatial data analysis in diverse domains of Earth and climate sciences \cite{HUANG2020104580,Arabameri}. Examples of such techniques include automated heuristic clustering \cite{8524429,10615674,669164,4666175}, flexible control of variables \cite{8999627}, optimised pattern recognition \cite{9708387,1634653}, ensemble learning \cite{10145147}, and objective analysis \cite{8938484,9279780}. 

%----------- section ----------->
\subsection{Objectives and goals}

{This study} presents an application of ML methods for image{ }classification using{ }Geographic Resources Analysis Support System (GRASS) GIS. {The ML algorithms} of GRASS GIS perform{ }automatic{, independent} pixel-level image classification{ }to objectively represent landscape dynamics. The main goal{ }is to{ identify} environmental { changes in land cover types }of Djibouti using { time series of }satellite images. {GRASS GIS is an advanced scripting cartographic and RS software with} powerful computational engine for geospatial processing \cite{NETELER2012124}. {Developed by the international GRASS developer community} \cite{grass_development_team_2025_14918754}, {it is used for processing cartographic data, maps, models and satellite image analysis across various applications in geoinformatics. Its important benefits consist in a built-in framework and Python Application Programming Interface (API) which supports automation of data processing through scripting.} To achieve this goal, the objective{ }is to map land cover changes in the coastal region of Djibouti{ }using RS data.{ }The{ }scripts are included and commented for{ }reference{. }The specific objectives of this research are as follows:
  ...
  Class & \rotatebox{90}{Salt plains} & \rotatebox{90}{\makecell{Barren \\Land}} & \rotatebox{90}{\makecell{Water\\ Bodies}} & \rotatebox{90}{Mangroves} & \rotatebox{90}{Shrubs} & \rotatebox{90}{Farmland} & \rotatebox{90}{\makecell{Built-up \\Areas}} & \rotatebox{90}{Forest} & \rotatebox{90}{Cropland}  & \rotatebox{90}{\makecell{Sparse \\Vegegation}} & \rotatebox{90}{Total} \\
   \midrule
    ...
    {6} & 22 & 16 & 39 & 0 & 2 & {546} & 5 & 8 & 31 & 83 & 752  \\
     ...
     \midrule 
     {} & \multicolumn{11}{c}{\hl{OA} = 93.4\%} \\ 
     \midrule
     {} &\multicolumn{11}{c}{\hl{KC} = 87.5\%} \\ 
    \bottomrule
\end{tabularx}
\end{table}
